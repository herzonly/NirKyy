...
router.get('/voicehero-ml', async (req,res) => {
  let { hero } = req.query;

const capitalizeFirstLetter = (str) => str ? str.charAt(0).toUpperCase() + str.slice(1).toLowerCase() : "";

if (!hero || typeof hero !== 'string') {
  return res.errorJson("Nama hero nya mana, woy?! Ga niat ya? Atau formatnya salah?", 400);
}

hero = capitalizeFirstLetter(hero);

if (hero.toLowerCase().includes('undefined')) {
  return res.errorJson("Nama heronya kok aneh gitu? Kayak ada 'undefined'-nya. Coba benerin dulu deh.", 400);
}

const encodedHero = encodeURIComponent(hero);
const url = `https://mobile-legends.fandom.com/wiki/${encodedHero}/Audio/id`;
...
try {
  const response = await axios.get(url, {
    headers: {
      ...
      'Referer': `https://mobile-legends.fandom.com/wiki/${encodedHero}/Audio/id`
    }
  });
  ...
  $('div.tabber > div.tabbertab').each((i, tabbertabElement) => {
    const $tabbertab = $(tabbertabElement);
    const skinName = $tabbertab.attr('title')?.trim();
    if (!skinName) return;
    ...
    const processAudioDiv = ($audioDiv) => {
      const $audioTag = $audioDiv.find('span.audio-button audio');
      const audioUrl = $audioTag.attr('src');
      
      const tempDivClone = $audioDiv.clone();
      tempDivClone.find('span.audio-button').remove();
      const quote = tempDivClone.text().trim();
      
      if (audioUrl && quote) {
        let finalCategory = currentMainCategory;
        
        if (currentSubCategory) {
          finalCategory = finalCategory ? `${finalCategory} - ${currentSubCategory}` : currentSubCategory;
        }
        
        const $prevDl = $audioDiv.prev('dl');
        if ($prevDl.length > 0) {
          const $dt = $prevDl.find('dt');
          if ($dt.length > 0) {
            const detailText = $dt.last().text().trim();
            if (detailText) {
              finalCategory = finalCategory ? `${finalCategory} - ${detailText}` : detailText;
            }
          }
        }
    ...
    $tabbertab.children().each((idx, childElement) => {
      const $child = $(childElement);
      
      if ($child.is('h2')) {
        currentMainCategory = ($child.find('.section-header-label').text() || $child.text()).replace(/\[edit\]\s*$/, '').trim();
        currentSubCategory = '';
      } else if ($child.is('h3')) {
        currentSubCategory = ($child.find('span.mw-headline').text() || $child.text()).replace(/\[edit\]\s*$/, '').trim();
      } else if ($child.is('div') && $child.children('span.audio-button').length > 0) {
        processAudioDiv($child);
      } else if ($child.is('section')) {
        $child.find('div').each((k, divInSectionElement) => {
          const $divInSection = $(divInSectionElement);
          if ($divInSection.children('span.audio-button').length > 0) {
            processAudioDiv($divInSection);
          }
        });
      }
    });
    
    if (audiosForSkin.length > 0) {
      allSkinsData.push({
        skin: skinName,
        audios: audiosForSkin
      });
    }
  });
  
  if (allSkinsData.length === 0) {
    return res.errorJson(`Gak nemu audio apa-apa buat hero "${hero}". Yakin heronya beneran ada? Ato mungkin emang heronya bisu kali ya.`, 404);
  }
  
  res.successJson(allSkinsData);
  
} catch (error) {
  console.error(`Error scraping for hero "${hero}": ${error.message}`);
  if (error.response && error.response.status === 404) {
    return res.errorJson(`Hero "${hero}" kayaknya gak ada di Fandom, Gj website nya emang`, 404);
  }
  res.errorJson("Error Jrit Pas akses website nya", 500);
}
});
