...
const snapsave = async (req, res) => {
  ...
  if (!url) {
    return res.errorJson({ status: false, code: 400, result: { error: "Linknya mana? Niat download kagak sih? 🗿" } });
  }

  const platform = isPlatform(url);
  if (!platform) {
    return res.errorJson({ status: false, code: 400, result: { error: "Linknya kagak valid tuh.. cobalah input yang bener 🗿" } });
  }
  ...
  try {
    ...
    if (!b) {
      return res.errorJson({ status: false, code: 404, result: { error: "Data encodenya kagak ada bree 🙃" } });
    }
    ...
    return res.succesJson({ status: true, code: 200, result: { platform, media: medias.result.media } });
