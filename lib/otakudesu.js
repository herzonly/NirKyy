...
async function fetchHtml(targetUrl, referer = null) {
  try {
    ...
    const response = await axios.get(targetUrl, { headers, timeout: 30000, maxRedirects: 10, validateStatus: s => s >= 200 && s < 500 });
...
function rewriteUrl(originalRelativeUrl) {
  if (!originalRelativeUrl || originalRelativeUrl === '#' || originalRelativeUrl.startsWith('http')) return originalRelativeUrl;
...
async function scrapeHome(page = 1) {
  ...
  $('.row .col-lg-2.col-md-3.col-sm-4.col-6').each((_, el) => {
    ...
    card.find('p.card-text strong, p.card-text').contents().each((_, node) => {
      ...
      else if (node.type === 'text' && info.currentKey) { info[info.currentKey] = $(node).text().trim(); info.currentKey = null; }
    });
    const display = {};
  ...
  if (paginationNav.length) {
    ...
    paginationNav.find('.page-item').each((_, el) => {
      ...
      else if (!isNaN(parseInt(txt, 10))) { pagination.pages.push({ number: parseInt(txt, 10), target_page: tp }); if ($(el).hasClass('active')) pagination.current_page = parseInt(txt, 10); }
    });
...
async function scrapeAnimeDetails(id) {
  ...
  $('.col-md-8 .my-3 p').each((_, el) => {
    ...
    let val = '';
    $(el).find('strong').nextAll().each((__, n) => { if (n.type === 'text') val += $(n).text(); });
    if (key) details.info[key] = val.trim() || 'N/A';
  });
  details.info.genre = [];
...
async function scrapeEpisodePage(id) {
  ...
  $('#streaming-source option').each((_, el) => {
    ...
    if (val && val !== 'default' && txt !== 'Default Source') data.stream_servers.push({ id: val, name: txt });
  });
...
async function searchAnime(q) {
  ...
  try {
    ...
    if (Array.isArray(data.results)) return data.results.map(r => ({ ...r, link: r.animeId ? rewriteUrl(`/anime/${r.animeId}`) : rewriteUrl(r.href) }));
  } catch {}
  return [];
}
...
module.exports = async (req, res) => {
  res.successJson = data => res.status(200).json(data);
  res.errorJson = (message, status = 400) => res.status(status).json({ error: message });
    ...
    switch (action) {
      ...
      case 'get_server_url':
        ...
        try {
          ...
          if (resp.data && resp.data.url) res.successJson(resp.data);
