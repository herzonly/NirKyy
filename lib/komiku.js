...
module.exports = async (req, res) => {
 ...
 if (html) {
     ...
     let currentMangaTitle = 'Komik'; // Default value
     let mangaDetailSlug = null;
     
     // Target the link inside the header paragraph specifically
     const breadcrumbMangaLink = $('#Judul header p a[href*="/manga/"]').first();
     if (breadcrumbMangaLink.length) {
         // Get text, potentially includes "Komik " prefix
         let rawTitle = breadcrumbMangaLink.text().trim();
         // Remove the "Komik " prefix if it exists
         currentMangaTitle = rawTitle.replace(/^Komik\s+/i, '').trim();
     ...
     $('#Judul table.tbl tbody tr').each((i, tr) => {
         ...
         // Exclude the main "Judul" row which is the chapter title + manga title
         if (key && value && key.toLowerCase() !== 'judul') {
             infoTableRows.push({ key, value });
         }
         // Removed the logic trying to get manga title from the table here
     });
     ...
     const prevLink = $('.botmenu .nxpr a.rl').first();
     const nextLink = $('.botmenu .nxpr a.rl').last();
     
     if (prevLink.length) {
         const prevHref = prevLink.attr('href');
         // Check if it's the actual previous link (caret-left icon)
         if (prevHref && !prevHref.includes('#') && prevLink.find('svg[data-icon="caret-left"]').length) {
             prevChapterSlug = extractChapterSlugFromUrl(prevHref);
         }
     }
     
     if (nextLink.length && nextLink[0] !== prevLink[0]) {
         const nextHref = nextLink.attr('href');
         // Check if it's the actual next link (caret-right icon)
         if (nextHref && !nextHref.includes('#') && nextLink.find('svg[data-icon="caret-right"]').length) {
             nextChapterSlug = extractChapterSlugFromUrl(nextHref);
         }
     } else if (prevLink.length && nextLink[0] === prevLink[0]) {
         // Handle case where only one link exists, check its icon to determine if it's next
         if (prevLink.find('svg[data-icon="caret-right"]').length) {
             const nextHref = prevLink.attr('href');
     ...
     resultData.data = {
         ...
         manga_title: currentMangaTitle, // Use the potentially cleaned title
