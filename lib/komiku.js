...
const PROXY_BASE_URL = 'https://nirkyy.koyeb.app/api/v1';
    ...
    try {
        if (searchQuery) {
            ...
            if (html) {
                ...
                if (mangaDetailSlug) {
                    const mangaUrl = `${BASE_URL}/manga/${mangaDetailSlug}/`;
                    const detailHtml = await fetchUrl(mangaUrl);
                    if (detailHtml) {
                        const $$ = cheerio.load(detailHtml);
                        const allChapterSlugs = [];
                        $$('#Daftar_Chapter #daftarChapter tr').slice(1).find('td.judulseries a').each((i, a) => {
                            const href = $$(a).attr('href');
                            const slug = extractChapterSlugFromUrl(href);
                            if (slug) {
                                allChapterSlugs.push(slug);
                            }
                        });
                        allChapterSlugs.reverse();

                        const currentChapterIndex = allChapterSlugs.indexOf(chapterSlug);
                        if (currentChapterIndex !== -1) {
                            if (currentChapterIndex > 0) {
                                prevChapterSlug = allChapterSlugs[currentChapterIndex - 1];
                            }
                            if (currentChapterIndex < allChapterSlugs.length - 1) {
                                nextChapterSlug = allChapterSlugs[currentChapterIndex + 1];
                            }
                        }
                    }
                }
