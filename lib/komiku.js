 ...
 if (html) {
     ...
     $('#Judul table.tbl tbody tr').each((i, tr) => {
         ...
         if (!mangaDetailSlug && key.toLowerCase() === 'judul komik') {
             const linkInValue = $(tr).find('td').eq(1).find('a').first();
             if (linkInValue.length) {
                 mangaDetailSlug = extractMangaSlug(linkInValue.attr('href'));
                 currentMangaTitle = value; // Keep the title from the table if slug found here
             } else {
     ...
     if (nextLink.length && nextLink[0] !== prevLink[0]) {
         ...
         // Handle case where only one link exists, check its icon
         if (prevLink.find('svg[data-icon="caret-right"]').length) {
             const nextHref = prevLink.attr('href');
             if (nextHref && !nextHref.includes('#')) {
                 nextChapterSlug = extractChapterSlugFromUrl(nextHref);
                 prevChapterSlug = null; // Reset prev slug as this was actually the next link
             }
         }
     }
