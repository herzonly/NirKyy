...
class DeepAi {
  constructor(apiKey = null) {
    this.endpoint = 'https://api.deepai.org/api/';
    this.userAgent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/134.0.0.0 Safari/537.36 Edg/134.0.0.0';
    this.apiKey = apiKey || this.generateTryItApiKey(this.userAgent);
  }
  ...
  async generateImage(text, { version = 'hd', oldModel = false, quality = true, geniusPreference = 'classic', style = 'anime-portrait-generator' } = {}) {
    const form = new FormData();
    form.append('text', text);
    form.append('image_generator_version', version);
    form.append('use_old_model', oldModel);
    form.append('quality', quality);
    form.append('genius_preference', geniusPreference);
    try {
      const response = await axios.post(this.endpoint + style, form, {
        headers: {
          'api-key': this.apiKey,
...
module.exports = async (req, res) => {
  ...
  const result = await deepai.generateImage(prompt);
  res.json(result);
};